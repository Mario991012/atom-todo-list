...
describe("UsersController", () => {
  ...
  describe("createUser", () => {
    ...
    it("should create a user and return status 201", async () => {
      ...
      (UsersService.prototype.createUser as jest.Mock).mockResolvedValue(newUser);

      await usersController.createUser(req as Request, res as Response);

      expect(UsersService.prototype.createUser).toHaveBeenCalledWith("test@example.com");
    ...
    it("should handle error and return status 500", async () => {
      ...
      req.body = { email: "test@example.com" };
      (UsersService.prototype.createUser as jest.Mock).mockRejectedValue(new Error(errorMessage));

      await usersController.createUser(req as Request, res as Response);

      expect(UsersService.prototype.createUser).toHaveBeenCalled();
      expect(logger.info).toHaveBeenCalledWith("findUser Response", expect.any(Error));
  ...
  describe("findUser", () => {
    it("should return 400 if email is not provided", async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith("Email required to find user", { email: undefined });
      expect(statusMock).toHaveBeenCalledWith(400);
      expect(jsonMock).toHaveBeenCalledWith({ message: "Email is required" });
    });
    ...
    it("should find a user and return status 200", async () => {
      ...
      (UsersService.prototype.findUserByEmail as jest.Mock).mockResolvedValue(user);

      await usersController.findUser(req as Request, res as Response);

      expect(UsersService.prototype.findUserByEmail).toHaveBeenCalledWith("test@example.com");
    ...
    it("should handle error and return status 404", async () => {
      ...
      req.params = { email: "test@example.com" };
      (UsersService.prototype.findUserByEmail as jest.Mock).mockRejectedValue(new Error(errorMessage));

      await usersController.findUser(req as Request, res as Response);

      expect(UsersService.prototype.findUserByEmail).toHaveBeenCalled();
      expect(logger.error).toHaveBeenCalledWith("findUser Response", expect.any(Error));
