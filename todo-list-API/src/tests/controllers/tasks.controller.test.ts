...
describe("TasksController", () => {
  ...
  describe("getAllTasks", () => {
    ...
    it("should handle error and return status 500", async () => {
      ...
      (TasksService.getAllTasks as jest.Mock).mockRejectedValue(new Error(errorMessage));

      await tasksController.getAllTasks(req as Request, res as Response);

      expect(TasksService.getAllTasks).toHaveBeenCalled();
      expect(logger.error).toHaveBeenCalledWith("getAllTasks error", errorMessage);
  ...
  describe("createTask", () => {
    it("should create a new task and return status 201", async () => {
      const newTaskMock = { id: 1, title: "New Task", description: "Task description" };
      req.body = {
        title: "New Task",
      ...
      expect(logger.info).toHaveBeenCalledWith("Task to be created", expect.objectContaining({
        title: "New Task",
        description: "Task description",
        completed: false,
        isDeleted: false,
    ...
    it("should handle error and return status 500", async () => {
      ...
      (TasksService.createTask as jest.Mock).mockRejectedValue(new Error(errorMessage));

      await tasksController.createTask(req as Request, res as Response);

      expect(TasksService.createTask).toHaveBeenCalled();
      expect(logger.error).toHaveBeenCalledWith("getAllTasks error", expect.any(Error));
      expect(statusMock).toHaveBeenCalledWith(500);
      expect(jsonMock).toHaveBeenCalledWith({
        error: errorMessage,
        returnCode: RETURN_CODES.GENERIC_ERROR,
  ...
  describe("updateTask", () => {
    it("should update the task and return status 200", async () => {
      const updatedTaskMock = { id: 1, title: "Updated Task", description: "Updated description" };
      req.params = { id: "1" };
      req.body = {
        title: "Updated Task",
      ...
      expect(TasksService.updateTask).toHaveBeenCalledWith("1", expect.objectContaining({
        title: "Updated Task",
        description: "Updated description",
        completed: true,
      }));
    ...
    it("should handle error and return status 500", async () => {
      ...
      (TasksService.updateTask as jest.Mock).mockRejectedValue(new Error(errorMessage));

      await tasksController.updateTask(req as Request, res as Response);

      expect(TasksService.updateTask).toHaveBeenCalled();
      expect(logger.error).toHaveBeenCalledWith("updateTask error", expect.any(Error));
      expect(statusMock).toHaveBeenCalledWith(500);
      expect(jsonMock).toHaveBeenCalledWith({
        error: errorMessage,
        returnCode: RETURN_CODES.GENERIC_ERROR,
  ...
  describe("deleteTask", () => {
    it("should delete the task and return status 200", async () => {
      ...
      req.params = { id: "1" };
      (TasksService.deleteTask as jest.Mock).mockResolvedValue(deleteResultMock);
    ...
    it("should handle error and return status 500", async () => {
      ...
      req.params = { id: "1" };
      (TasksService.deleteTask as jest.Mock).mockRejectedValue(new Error(errorMessage));

      await tasksController.deleteTask(req as Request, res as Response);

      expect(TasksService.deleteTask).toHaveBeenCalledWith("1");
      expect(logger.error).toHaveBeenCalledWith("deleteTask error", expect.any(Error));
      expect(statusMock).toHaveBeenCalledWith(500);
      expect(jsonMock).toHaveBeenCalledWith({
        error: errorMessage,
        returnCode: RETURN_CODES.GENERIC_ERROR,
